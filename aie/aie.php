...
elseif($func=="grad"){
	...
	if(!$error){
		...
		foreach($arr as $d){
			$src=$aie_dir."/grad.php?w=90&h=144&d=".$d."&s=".$s."&e=".$e;
			$str.="<div class='aie-grad'><label><img src='$src' title='Click to Choose' class='aie-grad-img' /><br /><input type='radio' name='grad-type' value='$d' /></label></div>";
			if($s==$e){break;}
		}
		$res="success|".$str;
	}
...
elseif($func=="init"){
	...
	if(!$error){
		...
		if($bg_s=="grad"){
			...
			if($chk){
				$im=new gd_gradient($w,$h,$d,$s,$e,0);
				$im->save($dir."/".$file);
			}
			else{
...
elseif($func=="text"){
	...
	if(!$error){
		...
		if($DB->connect==1){
			$id=$DB->getauto("text");
			$DB->query("insert into $dbprefix"."text set text_id='$id',user_id='$UserID',text_val='".addslashes($text)."',text_font='".addslashes($_POST["font"])."',text_size='$size',text_color='$color',text_align='$align',text_dir='".addslashes($_POST["key"])."',text_file='$file_n'");
		}
...
elseif($func=="save"){
	...
	if(!$error){
		...
		$file_n=$dir."/Graphics-".date("YmdHis").".png";
		$im->resizeImage($im->widthOriginal,$im->heightOriginal);
		$im->saveImage($file_n,100);

		if(!is_dir("../upload/".$UserID)){
			mkdir("../upload/".$UserID,0777);
			chmod("../upload/".$UserID,0777);
		}
		if(!is_dir("../upload/".$UserID."/flat")){
			mkdir("../upload/".$UserID."/flat",0777);
			chmod("../upload/".$UserID."/flat",0777);
		}

		$file_c=str_replace($dir."/","",$file_n);
		copy($file_n,"../upload/".$UserID."/flat/".$file_c);

		$DB=new db($dbhost,$dbuser,$dbpass,$dbname);
		$DB->connect();
		if($DB->connect==1){
			$DB->query("insert into $dbprefix"."cover set user_id='$UserID',cover_file='$file_c',cover_dir='".addslashes($_POST["key"])."',cover_bg='".addslashes($_POST["file"])."',cover_obj='".addslashes($_POST["str"])."',cover_rd='".time()."'");
			$DB->query("insert into $dbprefix"."limit set user_id='$UserID',limit_type='cover_flat',limit_rd='".time()."'");
		}

		$res="success|index.php?cmd=coverok&type=flat&name=".$file_c;
	}
	else{
		$res="No Template found.";
	}
...
elseif($func=="edit"){
	...
	$DB->connect();
	if($DB->connect==1){
		if($cur_cover=$DB->info("cover","cover_id='$id' and user_id='$UserID'")){
			$error=0;
		}
	}

	if(!$error){
		$arr=getimagesize($cur_cover["cover_bg"]);
		$res="success|".$cur_cover["cover_dir"]."|".$cur_cover["cover_bg"]."|".$arr[0]."|".$arr[1]."|".$cur_cover["cover_obj"];
	}
	else{
...
elseif($func=="textedit"){
	...
	$DB->connect();
	if($DB->connect==1){
		if(($cur_text=$DB->info("text","text_id='$id' and user_id='$UserID' and text_dir='$key' and text_file='$file'"))&&is_dir($dir)&&is_file($file)){
			$error=0;
		}
	}

	if(!$error){
		$res="success|".$cur_text["text_val"]."|".$cur_text["text_font"]."|".$cur_text["text_size"]."|".$cur_text["text_color"]."|".$cur_text["text_align"];
	}
	else{
		$res="No Text found.";
	}
