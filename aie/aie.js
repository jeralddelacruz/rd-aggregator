...
jQuery(document).ready(function($){
	$(".aie-thumb-tpl").click(function(){
		...
		$.ajax({
			type: "POST",
			url: aie_dir+"/aie.php",
			data: "func=load&dir=bg&sub="+$(this).text(),
			cache: false,
...
//	$("div#aie-grad").on("click","div.aie-grad",function(){
	$("#btnBgCreate").click(function(){
		if(isNaN($("#bg-w").val())||isNaN($("#bg-h").val())){
			alert("Invalid Background Size!");
			$("#bg-w").focus().select();
		}
		else{
			block();
			$.ajax({
				type: "POST",
				url: aie_dir+"/aie.php",
				data: "func=init&bg=grad&w="+$("#bg-w").val()+"&h="+$("#bg-h").val()+"&chk="+$("#bg-grad").val()+"&d="+$("input[name=grad-type]:checked").val()+"&s="+$("#aie-color-s").val()+"&e="+$("#aie-color-e").val(),
				cache: false,
				success: function(response){
					res=response.split("|");
					if(res[0]=="success"){
...
//						$("#img-"+iNum).attr({"width":res[2]+"px","height":res[3]+"px","bright":0,"contrast":0,"textID":res[4]});
	...
	$("#btnSave").click(function(){
		...
		$.ajax({
			type: "POST",
			url: aie_dir+"/aie.php",
			data: "func=save&key="+cur_dir+"&file="+$("#bg").attr("src")+"&str="+str,
			cache: false,
			success: function(response){
				res=response.split("|");
				if(res[0]=="success"){
...
//		window.location.href="index.php?cmd=cover";
//		window.close();
//		window.opener.focus();
		window.top.location.href="../user/index.php?cmd=cover";
	});
