<?php
	...
	$campaign = $DB->info("pageb", "pageb_campaign_unique_identifier_download = '{$passed_random_string}'");

	$plr_from_prb = $DB->info("prb", "prb_id = '{$campaign["pageb_campaign_lead_magnet_id"]}'");
	$plr_from_pr = $DB->info("pr", "pr_id = '{$plr_from_prb["pr_id"]}'");

	if($_GET["download"]){
		$download_id = $_GET["download"];

		$for_download = $DB->info("pr", "pr_id = '{$download_id}'");

		$url = $for_download["pr_url"];
		$_arr = pathinfo($url);
...
<html class="cs-bg-color-gainsboro">
	<head>
		<title><?= $campaign["pageb_campaign_title"]; ?></title>
	...
	<body class="cs-bg-color-gainsboro" style="height: auto;">
		<div class="cs-main-wrapper">
			<div class="cs-download-container cs-border-dashed cs-border-width-3px cs-border-color-darkgrey cs-border-radius-4px cs-margin-top-50px cs-bg-color-white">
				...
				<div class="cs-column-flex">
					<div class="cs-image-container cs-width-30percent cs-margin-bottom-30px">
						<img src="<?= $plr_from_pr["pr_cover"] ?>" />
					</div>

					<div class="cs-text-title cs-text-font-size-27px cs-margin-bottom-10px"><?= $plr_from_pr["pr_title"] ?></div>
					<div class="cs-text-body cs-text-font-size-20px cs-margin-bottom-30px"><?= $plr_from_pr["pr_desc"] ?></div>

					<a class="cs-download-button" href="<?= $SCRIPTURL . "add/download.php?s={$passed_random_string}&download={$plr_from_pr["pr_id"]}"; ?>"><i class="fa fa-arrow-down"></i> Click Here To Download Now</a>
				</div>
			</div>
		</div>
	</body>
